...
async function likeMessage(btn) {
    ...
    if (activeChatId) {
        ...
        try {
            const response = await fetch(`https://srv975554.hstgr.cloud:5000/chat/${activeChatId}/like`, {
                method: 'POST',
...
async function dislikeMessage(btn) {
    ...
    if (activeChatId) {
        ...
        try {
            const response = await fetch(`https://srv975554.hstgr.cloud:5000/chat/${activeChatId}/dislike`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message_index: messageIndex })
            });
...
async function retryMessage(btn) {
    ...
    try {
        const response = await fetch(`https://srv975554.hstgr.cloud:5000/chat/${activeChatId}/regenerate`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message_index: messageIndex })
        });
...
async function saveMessageEdit(btn, messageIndex) {
    ...
    try {
        if (activeChatId) {
            // Send to server
            const response = await fetch(`https://srv975554.hstgr.cloud:5000/chat/${activeChatId}/message/${messageIndex}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
