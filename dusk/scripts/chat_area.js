...
async function loadChatHistory() {
    try {
        const response = await fetch('https://srv975554.hstgr.cloud/chats');
...
async function loadChat(chatName) {
    try {
        const response = await fetch(`https://srv975554.hstgr.cloud/chat/${chatName}`);
        const data = await response.json();
...
async function handleChatModalAction() {
    ...
    if (currentChatAction === 'rename') {
        ...
        try {
            const resp = await fetch(`https://srv975554.hstgr.cloud/chat/${encodeURIComponent(currentChatId)}/rename`, {
                method: 'PUT',
        ...
        try {
            const resp = await fetch(`https://srv975554.hstgr.cloud/chat/${encodeURIComponent(currentChatId)}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
            const result = await resp.json();
