...
async function loadChatHistory() {
    try {
        const response = await fetch(Server_ip+'/chats');
...
async function loadChat(chatName) {
    try {
        const response = await fetch(Server_ip + `/chat/${chatName}`);
        const data = await response.json();
...
async function handleChatModalAction() {
    ...
    if (currentChatAction === 'rename') {
        ...
        try {
            const resp = await fetch(Server_ip + `/chat/${encodeURIComponent(currentChatId)}/rename`, {
                method: 'PUT',
        ...
        try {
            const resp = await fetch(Server_ip + `/chat/${encodeURIComponent(currentChatId)}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
            const result = await resp.json();
