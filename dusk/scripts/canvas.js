...
async function loadCanvasFromServer() {
    try {
        const response = await fetch('http://srv975554.hstgr.cloud:5000/canvas', {
            method: 'GET',
...
async function clearCanvasOnServer() {
    try {
        const response = await fetch('http://srv975554.hstgr.cloud:5000/canvas/clear', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
...
async function exportCanvas() {
    ...
    try {
        ...
        if (shouldSaveToServer) {
            try {
                const response = await fetch('http://srv975554.hstgr.cloud:5000/canvas/export', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
...
async function syncCanvasWithServer() {
    ...
    try {
        const response = await fetch('http://srv975554.hstgr.cloud:5000/canvas/sync', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
window.canvasMemory = {
    ...
    isServerAvailable: async () => {
        try {
            const response = await fetch('http://srv975554.hstgr.cloud:5000/canvas', { method: 'GET' });
            return response.ok;
        } catch {
            return false;
        }
